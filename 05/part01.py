...
#     for order in orderList:
#         if order[0] in bad_pages[p] and order[1] in bad_pages[p]:
#             left_idx = bad_pages[p].find(order[0])
#             righ_idx = bad_pages[p].find(order[1])
#             if left_idx > righ_idx:
#                 pa = bad_pages[p].replace(order[1]+',','')
#                 bad_pages[p] = pa[:righ_idx] + order[1]+','+ pa[righ_idx:]




# wrong_string_list = []
# for line in wrong_sequences:
#      wrong_string_list.append(",".join([str(l) for l in line]))
# secondList = goodNbaad(orderList, wrong_string_list)
# # wrong_string_list = [ ",".join(line) for line in wrong_sequences]

# good_pages = [page for page in secondList if '+' in page]
# baad_pages = [page.strip('x') for page in secondList if 'x' in page]
# print(len(good_pages), len(baad_pages), len(secondList))
...
# for rlist in relevantList[0]:
#     pass

# print(Fore.RED,relevantList[0][0],Fore.RESET)#, len(relevantList[0]))
# print(Fore.GREEN,relevantList[0][1],Fore.RESET)#,len(relevantList[0]))
